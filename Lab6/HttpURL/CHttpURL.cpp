...
void CHttpURL::AssertValidPort(const int port)
{
    if (port < MIN_PORT || port > MAX_PORT)
        throw std::invalid_argument("Invalid port");
}

void CHttpURL::AssertValidDomain(const std::string& domain)
{
    if (!std::regex_match(domain, DOMAIN_REGEX))
        throw std::invalid_argument("Invalid domain: " + domain);
}

Protocol CHttpURL::GetProtocolByString(std::string protocol)
{
    if (protocol == "http")
...
int CHttpURL::GetPortByProtocol(Protocol protocol)
{
...
std::string CHttpURL::CompleteDocument(const std::string& document)
{
...
std::string CHttpURL::BuildURL() const
{
    ...
    if ((m_port == HTTP_PORT && m_protocol == Protocol::HTTP) || (m_port == HTTPS_PORT && m_protocol == Protocol::HTTPS))
        return stringProtocol + "://" + m_domain + m_document;
    return stringProtocol + "://" + m_domain + ":" + std::to_string(m_port) + m_document;
}

std::string CHttpURL::GetStringByProtocol(Protocol protocol)
{
    if (protocol == Protocol::HTTP)
        return "http";
    return "https";
