...
CMyString& CMyString::operator=(const CMyString& a)
{
	...
	memcpy(m_string, a.m_string, a.m_length);
...
{
	char *resultString = (char*)malloc(sizeof(char) * (length + 1));
	memcpy(resultString, &m_string[start], length + 1);

	return CMyString(resultString, length);
}
...
std::istream& operator>> (std::istream& in, CMyString& a)
{
	...
	in >> tempString;
	memcpy(a.m_string, tempString.c_str(), a.m_length + 1);
